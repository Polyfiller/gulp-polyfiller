...
describe('gulp-polyfiller', function () {
	...
	beforeEach(function () {
		...
		polyfiller = require(index);
	});

	afterEach(function () {
		delete require.cache[path.resolve(__dirname, index)];
	});
	...
	describe('#main', function () {
		...
		it('should bypass files', function (done) {
			...
			streaming = polyfiller((features = [ chance.word() ]), {
			...
			process = new stream.Writable({
				objectMode: true
			});

			process._write = handler = sinon.spy(function (file, enc, callback) {
				callback(null, file);
			});
	...
	describe('#bundle', function () {
		...
		it('should write polyfills with process and wrapper', function (done) {
			...
			polyfiller.bundle(features, options)
				.pipe(through.obj(function (file, enc, callback) {
					try {
						...
						expect(stub.options.wrapper.firstCall
							.calledWithExactly(source)).to.be.true;
